...
from rctools.users import get_admin_user, get_admin_user_data_from_s3, update_admin_data_in_s3
...
@router.get('/payment/rates', status_code=status.HTTP_200_OK)
def get_installer_rates_by_state(X_Amz_Access_Token: Optional[str] = Header(default=None)) -> dict:
    ...
    uid = user['Username']
    installer = get_admin_user(cognito_client, s3_client, ADMIN_USER_POOL_ID, USER_DATA_BUCKET, uid)
...
def get_account_onboarding_link(X_Amz_Access_Token: Optional[str] = Header(default=None)):
    ...
    if not account_id:
        ...
        update_admin_data_in_s3(s3_client, USER_DATA_BUCKET, uid, {'payments': {'connected_account_id': account_id, 'refresh_token': refresh_token}})
    link = create_connected_installer_account_link(
        api_key,
        account_id,
        f'https://{PUBLIC_API_ROOT}/admin/onboarding/refresh?uid={uid}&refresh_token={refresh_token}',
...
def pay_for_premium_subscriptions(data: dict, X_Amz_Access_Token: Optional[str] = Header(default=None)) -> str:
    ...
    user = get_user_with_access_token(cognito_client, X_Amz_Access_Token)
    uid = user['Username']
    user_data = get_admin_user(cognito_client, s3_client, ADMIN_USER_POOL_ID, USER_DATA_BUCKET, uid)
    api_key = get_stripe_api_key()
    ...
    update_admin_data_in_s3(s3_client, USER_DATA_BUCKET, uid, {'payments': {**payments, **update}})
